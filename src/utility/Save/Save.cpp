...
//Serial.println("openFile");
	...
	if (blocks_old > blocks_new) {
		...
		for (uint16_t i = 0; i < blocks_new; i++) {
			...
			f.seek(SAVEHEADER_SIZE + blocks_old + i*4, SeekSet);
			f.read((uint8_t *)&b, 4);
			//f.SeekSet(SAVEHEADER_SIZE + blocks_new + i*4);
			f.seek(SAVEHEADER_SIZE + blocks_new + i*4, SeekSet);
		...
		f.seek(SAVEHEADER_SIZE + (blocks_old * 5) + payload_size, SeekSet);
		...
		for (uint32_t i = 0; i < payload_size; i++) {
			uint8_t b;
			f.seek(SAVEHEADER_SIZE + (blocks_old * 5) + (payload_size - i - 1), SeekSet);
			f.read((uint8_t *)&b, 1);
			f.seek(SAVEHEADER_SIZE + (blocks_new * 5) + (payload_size - i - 1), SeekSet);
			f.write((const uint8_t *)&b, 1);
		}
		...
		for (uint16_t i = 0; i < blocks_old; i++) {
			...
			f.seek(SAVEHEADER_SIZE + blocks_old + (blocks_old - i - 1)*4, SeekSet);
			f.read((uint8_t *)&b, 4);
			f.seek(SAVEHEADER_SIZE + blocks_new + (blocks_old - i - 1)*4, SeekSet);
			f.write((const uint8_t *)&b, 4);
		}
		
		// finally we nullate the new block metadata
		f.seek(SAVEHEADER_SIZE + blocks_old, SeekSet);
...
//Serial.println("_get");
	...
	f.seek(SAVEHEADER_SIZE + blocks + (4*i), SeekSet);
...
//Serial.println("get2");
	...
	f.seek(SAVEFILE_PAYLOAD_START + b, SeekSet);
...
saveFlag = true;
	...
	f.seek(SAVEFILE_PAYLOAD_START + payload_size, SeekSet);
	// write the size
...
//Serial.println("Del");
	...
	for (uint32_t j = 0; j < (payload_size-b-size); j++) {
		...
		f.seek(SAVEFILE_PAYLOAD_START + b + size + j, SeekSet);
		f.read((uint8_t *)&c, 1);
		//f.SeekSet(SAVEFILE_PAYLOAD_START + b + j);
		f.seek(SAVEFILE_PAYLOAD_START + b + j, SeekSet);
		f.write(c);
	}
