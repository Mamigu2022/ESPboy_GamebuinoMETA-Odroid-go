	...
	while (_headP != targetHeadP) {
		...
		if (_current_Sound_FX_time < _current_Sound_FX_length) {
			...
			if (_firstZeroP == nullptr) {
				...
				maxSamples = min_(maxSamples, _firstZeroP - _headP);
			}
...
void Sound_Handler_FX::generateNoise(uint32_t* endP) {
	...
	do {
		...
		if (--_noise_period <= 0) {
			_noise_period = max_((getFrequency() >> (FPP + 4)) / SR_DIVIDER, 0);
	...
	do {
		...
		if (_square_period <= (4 << FPP)) {
			for (int i = 0; i < 4; i++) {
				...
				if (_square_period <= 0) {
					_square_period += max_(getFrequency() / SR_DIVIDER, 0);
