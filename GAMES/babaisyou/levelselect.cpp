...
void set_page(uint8_t page)
{
  ...
  levels_on_page = min__(level_count_sd - start_level, levels_per_page);

  memset(buffer1, 0x00, ROW_SIZE);
  drawText(32, 4, "B: BACK TO TITLE", buffer1);
  if (current_page > 0)
  {
    drawText(20, 4, "<", buffer1);
  }
  if (current_page < last_page)
  {
    drawText(135, 4, ">", buffer1);
  }
...
void levelSelectTick()
{
  ...
  else if (gb.buttons.pressed(BUTTON_RIGHT))
  {
    if (selected_level % 2)
    {
      ...
      {
        selected_level--;
        selected_level = min__(selected_level, levels_on_page - 1);
  ...
  for (uint8_t row = 0; row < ((levels_per_page + 1) / 2); row++)
  {
    ...
    {
      drawText(27, 4, names[start_level + row * 2], buffer1);
      if (row * 2 + 1 < levels_on_page)
      {
        drawText(92, 4, names[start_level + row * 2 + 1], buffer1);
      }

      if (selected_level >> 1 == row)
      {
        uint8_t column = 18 + (selected_level % 2) * 65;
