...
extern uint8_t level[MAP_FULL_SIZE];
extern PROGMEM const uint16_t FONT[];
extern bool isYou(uint8_t tile);
...
{ 
  ...
  if (CURRENT_SCREEN != GAME_SCREEN) delta=5;
  delta*=3;
  if(delta>32)delta=32;
  
  if(buffer==&buffer1[0]){
    for(uint8_t i=0; i<h; i++) memcpy(&buffer3[i*128],&buffer1[i*160+delta],128*2);
    }
  else{
    for(uint8_t i=0; i<h; i++) memcpy(&buffer3[i*128],&buffer2[i*160+delta],128*2);
    }

  //int16_t x = 0;
  //int16_t w = 160;

  drawRectangle(0, y, 128, h, buffer3);
}

void drawRectangle(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t* buffer)
{
  drawPending = true;
  gb.tft.setAddrWindow(x, y, x + w, y + h);
