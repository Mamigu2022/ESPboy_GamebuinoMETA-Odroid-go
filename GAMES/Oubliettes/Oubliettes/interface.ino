...
// ###########################################################################
 void drawMain() {
  if (!music_on) {
    //sound_main = gb.sound.play("/Oubliettes/audio/MAIN.WAV", true);
    sound_main = gb.sound.play(mainMus, mainMus_size, true);
    music_on = true;
  }
...
void drawGameover() { 
  if (!music_on) {
   // sound_gameover = gb.sound.play("/Oubliettes/audio/GAMEOVER.WAV", false);
    sound_gameover = gb.sound.play(gameoverMus, gameoverMus_size, false);
    music_on = true;
  }
