...
void loadScore() {
  ...
  for (byte i = 0; i < 7; i++) {
    highscore[i]=gb.save.get(offset+i);
    gb.save.get(offset+7+i, (uint8_t *)scorename, 10);
  }
}
...
void saveScore() {
  ...
  for (byte j = 0; j < 7; j++) {
    if (score > highscore[j]) {
      ...
      char temp_scorename[9+1];
      strcpy(temp_scorename, scorename[j+1]); //workaround
      gb.getDefaultName( scorename[j] );       //funktioniert nicht richtig, überschreibt [j+1] mit "n"
      strcpy(scorename[j+1], temp_scorename); //workaround
      ...
      if ( gamemode )
        ...
        offset = 23; //blocks 23-37 easy highscore
      gb.save.set(offset, (uint8_t *)highscore, 14);
      for (byte i = 0; i < 7; i++) {
        gb.save.set(offset+i, highscore[i]);
        gb.save.set(offset+7+i, (uint8_t *)scorename,10);
      }
    }
  }
}
...
void loop() {
    while (!gb.update()) delay(1);
    
    switch (gamestate) {
      case PAUSED:
        gb.lights.clear();
        switch ( gb.gui.menu(lang_title, gamemode ? Pausemenu_hard : Pausemenu_easy, MENULENGTH, 3) ) {
