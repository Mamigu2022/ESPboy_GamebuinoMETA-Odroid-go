/*
   (C) Copyright 2017 Aurélien Rodot. All rights reserved.
...
#define EEPROM_TOKEN 0xFE
#define EEPROM_WEAPONS_OFFSET 0x004+EEPROM_OFFSET
#define EEPROM_MAPS_OFFSET 0x006+EEPROM_OFFSET
#define EEPROM_SCORE_OFFSET 0x0020+EEPROM_OFFSET
    ...
    void draw() {
      if (timeLeft) {
        ...
        if (subtype == W_EXPLOSION) {
          ...
          g = r = random (128, 255);
          b = r / 2;
          //gb.lights.fill((Gamebuino_Meta::Color)gb.createColor(r, g, b));
          gb.lights.setRGB(r/10,g/10,b/10);
    ...
    void update() {
      for (byte j = 0; j < NUMBULLETS; j++) {
        ...
        for (byte i = 0; i < NUMENEMIES; i++) {
          ...
          if (gb.collide.rectRect(enemies[i].x, enemies[i].y, enemies[i].getWidth(), enemies[i].getHeight(),
...
class Crate :
  ...
  public:
    ...
    void update() {
      ...
      if (gb.collide.rectRect(x, y, getWidth(), getHeight(),
...
//AUTOSAVE every 10sec
   if(millis()-gametimer>5000){
    ...
    for (byte i = 0; i < NUMBULLETS; i++) {
      ...
      if (bullets[i].damagePlayer() && gb.collide.rectRect(bullets[i].x, bullets[i].y, bullets[i].getWidth(), bullets[i].getHeight(),
          player.x, player.y, player.getWidth(), player.getHeight())) {
...
void loadEEPROM() {
  ...
  if (EEPROM.read(EEPROM_OFFSET) != EEPROM_TOKEN || EEPROM.read(EEPROM_OFFSET+1) != EEPROM_TOKEN) {
    cleanEEPROM();
    EEPROM.write(EEPROM_OFFSET, EEPROM_TOKEN);
    EEPROM.write(EEPROM_OFFSET+1, EEPROM_TOKEN);
    EEPROM.commit();
  }
  //load score for each map
  for (int i = 0; i < NUMMAPS; i++) {
    EEPROM.get(i * 2 + EEPROM_SCORE_OFFSET, score[i]);
  }
...
void saveEEPROM() {
  ...
  for (byte i = 0; i < NUMMAPS; i++) {
    uint16_t readedval;
    EEPROM.get(i * 2 + EEPROM_SCORE_OFFSET, readedval);
    if (readedval < score[i]) {
      EEPROM.put(i * 2 + EEPROM_SCORE_OFFSET, score[i]);
    }
  }
...
/*
uint16_t EEPROMreadInt(unsigned int i) {
  uint16_t value = EEPROM.read(i + 1) & 0x00FF; //LSB
...
void cleanEEPROM() {
  for (int i = EEPROM_OFFSET; i < 256; i++) {
      EEPROM.write(i, 0);
  }
  EEPROM.commit();
}
