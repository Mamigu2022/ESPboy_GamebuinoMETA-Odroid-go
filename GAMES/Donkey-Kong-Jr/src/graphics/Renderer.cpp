...
void Renderer::customDrawBuffer(uint8_t x, uint8_t y, uint16_t * buffer, uint8_t w, uint8_t h) {
  ...
  uint16_t *bufPointerWrite;
  uint16_t *bufPointerRead;
  
  gb.tft.setAddrWindow(0, y, 128, SliceHeight); 
  
  for (uint8_t j=0; j<SliceHeight; j++){
    bufPointerWrite = bufline;
    bufPointerRead = &buffer1[j*ScreenWidth];
    for (uint8_t k=0; k<32; k++){	
      *bufPointerWrite++ = *bufPointerRead++;
      *bufPointerWrite++ = *bufPointerRead++;
      *bufPointerWrite++ = *bufPointerRead++;
      *bufPointerWrite++ = gb.tft.colorsBlend(*bufPointerRead++, *bufPointerRead++);    
    }
    gb.tft.pushColors(bufline, 128, 0);
  }
}
...
void Renderer::draw(Game * engine) {
    ...
    for (uint8_t sliceIndex = 0; sliceIndex < slices; sliceIndex++) {
        ...
        buffer = buffer1;//sliceIndex % 2 == 0 ? buffer1 : buffer2;
