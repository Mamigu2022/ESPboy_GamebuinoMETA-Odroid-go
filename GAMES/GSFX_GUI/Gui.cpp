...
void __assert(const char *__func, const char *__file, int __lineno, const char *__sexp, const char * reason) {
	...
	if (SerialUSB)
	{
		Serial.println("=== ASSERTION ERROR ===");
...
Gamebuino_Meta::Sound_FX sfx_test[8] =
{
	...
	{ Gamebuino_Meta::Sound_FX_Wave::SQUARE,1, 0xFF ,0,120,0,5 },
...
void Gui::OutputPattern(const Gamebuino_Meta::Sound_FX *fx, uint16_t id)
{
		...
		Serial.print("//Put this in your .h file :\n");
		Serial.print("extern const Gamebuino_Meta::Sound_FX ");
		Serial.print(buffer);
		Serial.print("[];\n\n");

		Serial.print("//Put this in your .cpp file :\n");
		Serial.print("const Gamebuino_Meta::Sound_FX ");
		Serial.print(buffer);
		Serial.print("[] = {\n");
		int i = 0;
...
void Gui::OutputFX(const Gamebuino_Meta::Sound_FX * fx)
{
		...
		Serial.print(_output_wave_names[fx->type]);
		
		Serial.print(",");
		Serial.print(fx->continue_flag);

		Serial.print(",");
		Serial.print(fx->volume_start);

		Serial.print(",");
		Serial.print(fx->volume_sweep);

		Serial.print(",");
		Serial.print(fx->period_sweep);

		Serial.print(",");
		Serial.print(fx->period_start);

		Serial.print(",");
		Serial.print(fx->length);
...
void Gui::UpdateParams()
{
	...
	// Pattern select sutff
	{
		if (gb.buttons.pressed(BUTTON_A) && _current_index == (uint32_t)Gui::UI::PATTERN_SELECT)
		{
			...
			{
				_pattern_length = max_(1, (_pattern_length)-1);
			...
			{
				_pattern_length = min_(8, _pattern_length + 1);
